...
```{r save-data, eval = FALSE, warning=FALSE} 
## use saveRDS() to save each object as a .rds file 

saveRDS(object = df1, "data/raw_data/df_department.rds")

saveRDS(object = df2, "data/raw_data/df_retail.rds")
...
df_total <- df_department %>%
  left_join(df_retail, by = c("business" = "business", "year" = "year", "n" = "n"))

```

#### Plot results

plot results from each data set and then combine
